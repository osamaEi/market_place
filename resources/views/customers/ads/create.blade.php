                    ...
                    <div class="subcategory-list collapse mt-2" id="subcategory-{{ $category['type'] }}-{{ $category['id'] }}">
                        @foreach($category['children'] as $subcategory)
                            ...
                            @if($category['type'] === 'category1')
                                <a href="{{ route('ads.createHouseAds', ['cat_id' => $subcategory->id, 'type' => $category['type']]) }}" class="btn btn-secondary w-100 mb-1 text-start">
                                    {{ $subcategory->title }}
                                </a>
                            @elseif($category['type'] === 'category2')
                                <a href="{{ route('ads.createElectronicAds', ['cat_id' => $subcategory->id, 'type' => $category['type']]) }}" class="btn btn-secondary w-100 mb-1 text-start">
                                    {{ $subcategory->title }}
                                </a>
                            @elseif($category['type'] === 'category3')
                                <a href="{{ route('ads.createCarAds', ['cat_id' => $subcategory->id, 'type' => $category['type']]) }}" class="btn btn-secondary w-100 mb-1 text-start">
                                    {{ $subcategory->title }}
                                </a>
                            @elseif($category['type'] === 'category4')
                                <a href="{{ route('ads.createBikeAds', ['cat_id' => $subcategory->id, 'type' => $category['type']]) }}" class="btn btn-secondary w-100 mb-1 text-start">
                                    {{ $subcategory->title }}
                                </a>
                            @elseif($category['type'] === 'category5')
                                <a href="{{ route('ads.customer.createAds', ['cat_id' => $subcategory->id, 'type' => $category['type']]) }}" class="btn btn-secondary w-100 mb-1 text-start">
                                    {{ $subcategory->title }}
                                </a>
                            @endif
                        @endforeach
